...
const TRANSLATIONS = {
  'zh-CN': {
    ...
    // ─── Projects view ────────────────────────────────────────
    'projects.title': '我的项目',
    'projects.subtitle': '每个项目都是独立的比赛环境，选手和积分互不影响。',
    'projects.empty': '暂无项目。创建一个开始使用。',
    'projects.btn.create': '新建项目',
    'projects.btn.rename': '重命名',
    'projects.btn.delete': '删除',
    'projects.meta': '{players} 名选手 · {events} 场比赛',
    'projects.default_name': '我的比赛',
    'projects.prompt.create': '项目名称：',
    'projects.prompt.rename': '新名称：',
    'projects.confirm.delete': '删除项目「{name}」？所有选手、历史记录和当前比赛将永久丢失。',
    'projects.confirm.delete2': '确认删除？此操作不可撤销。',
    'projects.sort.label': '排序：',
    'projects.sort.recent': '最近',
    'projects.sort.name': '名称',
    'projects.sort.players': '人数',
    'projects.io.title': '全部导入 / 导出',
    'projects.io.export': '📤 导出所有项目',
    'projects.io.import': '📥 导入所有项目',
    'projects.io.hint': '导出包含所有项目及其数据。导入将替换所有现有内容。',
    'projects.import.confirm': '导入将用备份中的 {count} 个项目替换所有现有项目。继续？',
    ...
    // ─── Match prediction ───────────────────────────────────
    'predict.title': '比赛结果预测',
    'predict.hint': '点击选手分配到 A 队或 B 队，查看胜负概率预测。',
    'predict.team_counts': 'A 队 {a} 人 · B 队 {b} 人',
    'predict.btn.reset': '清空选择',
    'predict.need_both': '请为两队各选至少一名选手。',
    'predict.method.h2h': '基于对位数据（{games} 场）',
    'predict.method.winrate': '基于综合胜率',
    'predict.method.blended': '综合对位（{games} 场）和胜率',
    'predict.method.insufficient': '数据不足，无法预测。选手需要有至少三场的比赛记录。',
    'predict.a_win': 'A 队胜',
    'predict.draw': '平局',
    'predict.b_win': 'B 队胜',
    ...
    'setup.section.attendees': '本周参赛人员 & 消费',
    'setup.attendees.selected': '已选 {n} 人',
    'setup.attendees.quickadd.placeholder': '快速添加新选手',
    'setup.section.params': '队伍生成模式',
    'setup.params.team_size': '每队人数',
    'setup.params.courts': '场地数',
    'setup.params.duration': '每场时长（分）',
    'setup.params.total': '总时间（分）',
    'setup.section.expense': '本周消费',
    'setup.expense.hint': '将均摊到本周所有参赛人员',
    'setup.btn.form': '生成队伍',
    'setup.section.recordonly': '直接记录模式',
    'setup.recordonly.hint': '跳过自动组队，直接手动添加比赛并录入结果。每场比赛独立选择是否计入积分；胜率与参与度始终照常统计。',
    'setup.btn.recordonly': '开始记录',
    'setup.alert.need_players': '至少需要 {n} 人才能组队。',
    'setup.empty.db': '先在"选手数据库"中添加选手。',

    // ─── Record-only mode ────────────────────────────────────
    'record.title': '直接记录模式',
    'record.event.info': '日期 {date} · 共 {attendees} 人参与 · 消费 {expense}',
    'record.add.players.hint': '点击选手名字依次切换：未选 → A队 → B队 → 未选。所有比赛在点击"完成 & 提交所有比赛"前都可随时编辑或删除。',
    'record.add.counts_line': 'A 队 {a} 人 · B 队 {b} 人',
    'record.add.counts': '本场计入积分（取消勾选则只更新胜率，不发放积分）',
    'record.list.empty': '尚未添加任何比赛，点击下方"添加一场比赛"开始记录。',
    'record.btn.add_match': '➕ 添加一场比赛',
    'record.btn.finish': '完成 & 提交所有比赛',
    'record.alert.need_attendees': '请先在上一页选择本周参赛人员（至少 2 人）。',
    'record.alert.need_both_teams': 'A 队和 B 队都至少要有一名选手。',
    'record.alert.need_result': '请录入比赛结果（点 A胜 / 平局 / B胜，或直接填比分）。',
    'record.alert.match_invalid': '第 {n} 场尚未填写完整：{reason}',
    'record.confirm.remove': '删除这场比赛？',
    'record.confirm.finish': '确定完成并保存？所有已记录的比赛将一次性写入数据库 —— 胜率和参与度都会更新，标记"计入积分"的比赛同时发放积分。',
    'record.match.label': '第 {n} 场',
    'record.match.team_a': 'A 队',
    'record.match.team_b': 'B 队',
    'record.match.team_a_n': '第 {n} 场 A 队',
    'record.match.team_b_n': '第 {n} 场 B 队',
    'record.match.kind_ranked': '计入积分',
    'record.match.kind_friendly': '不计积分',
  ...
  'en-US': {
    ...
    'projects.subtitle': 'Each project is an independent tournament environment with its own players and stats.',
    'projects.empty': 'No projects yet. Create one to get started.',
    'projects.btn.create': 'New Project',
    'projects.btn.rename': 'Rename',
    'projects.btn.delete': 'Delete',
    'projects.meta': '{players} players · {events} events',
    'projects.default_name': 'My Tournament',
    'projects.prompt.create': 'Project name:',
    'projects.prompt.rename': 'New name:',
    'projects.confirm.delete': 'Delete project "{name}"? All players, history, and current event will be permanently lost.',
    'projects.confirm.delete2': 'Really delete? This cannot be undone.',
    'projects.sort.label': 'Sort:',
    'projects.sort.recent': 'Recent',
    'projects.sort.name': 'Name',
    'projects.sort.players': 'Players',
    'projects.io.title': 'Import / Export All',
    'projects.io.export': '📤 Export all projects',
    'projects.io.import': '📥 Import all projects',
    'projects.io.hint': 'Export includes all projects and their data. Import replaces everything.',
    'projects.import.confirm': 'Import will replace ALL existing projects with {count} project(s) from the backup. Continue?',
    ...
    // ─── Match prediction ───────────────────────────────────
    'predict.title': 'Match Prediction',
    'predict.hint': 'Tap players to assign to Team A or Team B for a win/draw/loss probability estimate.',
    'predict.team_counts': 'Team A: {a} · Team B: {b}',
    'predict.btn.reset': 'Clear selection',
    'predict.need_both': 'Select at least one player per team.',
    'predict.method.h2h': 'Based on head-to-head ({games} games)',
    'predict.method.winrate': 'Based on overall win rates',
    'predict.method.blended': 'Blended H2H ({games} games) + win rates',
    'predict.method.insufficient': 'Insufficient data for prediction. Each player needs at least 3 games on record.',
    'predict.a_win': 'Team A wins',
    'predict.draw': 'Draw',
    'predict.b_win': 'Team B wins',
    ...
    'setup.section.expense': "This week's expense",
    'setup.expense.hint': 'Will be split equally across attendees',
    'setup.btn.form': 'Generate teams',
    'setup.section.recordonly': 'Record-only mode',
    'setup.recordonly.hint': "Skip team formation and log matches by hand. Each match independently chooses whether it counts for tournament points; win rate and attendance are always recorded.",
    'setup.btn.recordonly': 'Start recording',
    'setup.alert.need_players': 'Need at least {n} players to form teams.',
    'setup.empty.db': 'Add players in "Player Database" first.',

    // ─── Record-only mode ────────────────────────────────────
    'record.title': 'Record-only mode',
    'record.event.info': '{date} · {attendees} attendees · spent {expense}',
    'record.add.players.hint': 'Tap a player to cycle: unselected → Team A → Team B → unselected. Every match stays editable until you tap "Finish & submit all matches".',
    'record.add.counts_line': 'Team A: {a} · Team B: {b}',
    'record.add.counts': "This match counts for points (uncheck for win-rate-only)",
    'record.list.empty': 'No matches recorded yet — tap "Add a match" below to start.',
    'record.btn.add_match': '➕ Add a match',
    'record.btn.finish': 'Finish & submit all matches',
    'record.alert.need_attendees': 'Pick at least 2 attendees on the previous screen first.',
    'record.alert.need_both_teams': 'Both Team A and Team B need at least one player.',
    'record.alert.need_result': 'Pick a result (A wins / Draw / B wins, or enter scores).',
    'record.alert.match_invalid': 'Match {n} is incomplete: {reason}',
    'record.confirm.remove': 'Remove this match?',
    'record.confirm.finish': 'Finish and save? All recorded matches commit to the database in one go — win rate and attendance update for everyone, and matches marked "counts for points" also award tournament points.',
    'record.match.label': 'Match {n}',
    'record.match.team_a': 'Team A',
    'record.match.team_b': 'Team B',
    'record.match.team_a_n': 'Match {n} Team A',
    'record.match.team_b_n': 'Match {n} Team B',
    'record.match.kind_ranked': 'counts',
    'record.match.kind_friendly': 'no points',
