...
- **5 种比赛模式**让用户每场自选：**自动**（推荐 —— 依次尝试小组赛+淘汰、循环赛、纯友谊）、**小组赛 + 淘汰赛**、**纯小组赛（循环赛）**、**纯淘汰赛**（2 的幂次队伍数，标准种子）、**纯友谊赛**（公平随机配对，不计积分）
...
- **纯友谊赛**（`planFriendly`）：包装 `planRandomFairFallback`，把每场都标记为 `kind: 'friendly'`。`commitEvent` 的累加器跳过非 ranked 比赛，所以友谊赛会记录结果方便用户查看，但**不影响任何选手的积分 / 胜 / 平 / 负**。
