...
- **纯单败淘汰赛**：标准网球种子排位（`seedBracket()`），头号种子和二号种子分在两半区，只能在决赛相遇。要求队伍数为 2 的幂次（2/4/8/16/32）—— 非 2 的幂次会被显式拒绝，让用户改模式而不是引入难看的轮空结构。
- **纯友谊赛**（`planFriendly`）：包装 `planRandomFairFallback`，把每场都标记为 `kind: 'friendly'`。`commitEvent` 的累加器对友谊赛传 `countPoints=false`，**胜 / 平 / 负仍然会更新**（所以胜率榜和对位胜率会反映出来），但**不会发放比赛积分**（所以积分榜不变）。对位胜率（H2H）和历史详情同理。
- **模式分发**：单一入口 `planByMode(mode, opts)` 路由到对应规划器。`auto` 模式保留历史的"智能选择"行为（小组赛+淘汰 → 循环赛 → 友谊赛）。其他四种是显式选择 —— 不可行时 UI 会提示并阻止下一步，不会偷偷切换。
